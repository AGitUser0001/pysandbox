...
class CoreTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_sandbox_process_health_and_shutdown(self) -> None:
        executable = (
            ...
            / "target"
            / "debug"
            / ("pysandbox-sandboxd.exe" if sys.platform == "win32" else "pysandbox-sandboxd")
        )
        self.assertTrue(executable.exists(), f"missing sandbox daemon: {executable}")
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            sandbox = await _core.start_sandbox(
                ...
                project_root / "component" / "runtime",
            )
