...
async def one_shot_demo(runtime: PythonRuntime) -> None:
  ...
  paragraph(
    "Guest code is sent on stdin to WASI CPython. The runtime prepends an API import, so exposed host RPC functions are available as ordinary callables."
  )
  step("demo")
...
async def worker_demo(runtime: PythonRuntime) -> None:
  ...
  paragraph(
    "A worker keeps guest state alive. The host can call functions or methods inside that worker through the same RPC channel."
  )
  step("demo")
  step("[worker]")
...
def playground() -> None:
  section("playground")
  paragraph(
    "Write a host Python program from start to end. The program can create a runtime, execute one-shot guest code, start long-lived guest workers, or expose host functions for the guest to call."
  )
  typewrite(
    f"{DIM}No runtime is created yet. Use Alt-1 or write PythonRuntime(...) yourself.{RESET}"
  )
...
class TerminalBlockEditor:
  ...
  def cursor_screen_position(self) -> tuple[int, int]:
    ...
    column = width + 3 + left_marker_columns + (self.cursor_column - self.viewport_left)
    return row, column
...
async def tutorial() -> None:
  ...
  try:
    ...
    paragraph(
      "This demo runs WASI CPython in an isolated sandbox process, exposes host functions over RPC, captures interlaced stdout/stderr, and then starts a long-lived guest worker."
    )

    section("host setup")
    paragraph(
      "Create a runtime and expose trusted host functions. The guest can call these by name."
    )
    step("demo")
    ...
    if not run_prompt():
      return
    typewrite(
      f"{GREEN}runtime ready{RESET}: exposed methods = {', '.join(runtime.rpc.methods)}"
    )
    if not continue_prompt():
      return
